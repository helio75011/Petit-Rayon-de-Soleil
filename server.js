 ...
 *   - Proposition de message          -> POST /api/messages (approved = 0)
...
init(); // s'assure que les tables existent au démarrage
...
app.put('/api/me', auth.requireAuth, (req, res) => {
  const { name } = req.body || {};
...
app.get('/api/messages', (req, res) => {
  const rows = db
    .prepare('SELECT id, content, created_at FROM messages WHERE approved = 1 ORDER BY id DESC')
    .all();
...
app.get('/api/messages/random', (req, res) => {
  const row = db
    .prepare('SELECT id, content FROM messages WHERE approved = 1 ORDER BY RANDOM() LIMIT 1')
    .get();
...
app.post('/api/messages', auth.requireAuth, (req, res) => {
  ...
  const info = db
    .prepare('INSERT INTO messages (content, approved, propose_par) VALUES (?, 0, ?)')
...
app.get('/api/favoris', auth.requireAuth, (req, res) => {
  const rows = db
    .prepare(
...
app.post('/api/favoris', auth.requireAuth, (req, res) => {
  const { message_id } = req.body || {};
  const msg = db.prepare('SELECT id FROM messages WHERE id = ? AND approved = 1').get(message_id);
  if (!msg) return res.status(404).json({ error: 'Message introuvable.' });
  // INSERT OR IGNORE : pas d'erreur si le favori existe déjà.
  db.prepare('INSERT OR IGNORE INTO favoris (user_id, message_id) VALUES (?, ?)').run(
    req.user.id,
    message_id
  );
...
app.delete('/api/favoris/:id', auth.requireAuth, (req, res) => {
  db.prepare('DELETE FROM favoris WHERE user_id = ? AND message_id = ?').run(
    req.user.id,
    req.params.id
  );
  res.json({ ok: true });
...
// POST /api/admin/approve/:id — approuver une proposition.
app.post('/api/admin/approve/:id', auth.requireAdmin, (req, res) => {
  const info = db.prepare('UPDATE messages SET approved = 1 WHERE id = ?').run(req.params.id);
  if (info.changes === 0) return res.status(404).json({ error: 'Message introuvable.' });
  res.json({ ok: true });
});

// DELETE /api/admin/reject/:id — refuser (supprimer) une proposition.
app.delete('/api/admin/reject/:id', auth.requireAdmin, (req, res) => {
  db.prepare('DELETE FROM messages WHERE id = ? AND approved = 0').run(req.params.id);
  res.json({ ok: true });
});
...
// On n'écoute pas pendant les tests (ils importent app sans démarrer le serveur).
