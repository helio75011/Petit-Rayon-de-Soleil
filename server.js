...
app.post('/api/logout', auth.requireAuth, wrap(async (req, res) => {
...
app.put('/api/me', auth.requireAuth, wrap(async (req, res) => {
  const { name } = req.body || {};
...
app.delete('/api/me', auth.requireAuth, wrap(async (req, res) => {
...
app.get('/api/messages', wrap(async (req, res) => {
  const rows = await db
    .prepare("SELECT id, content, created_at FROM messages WHERE status = 'approved' ORDER BY id DESC")
    .all();
...
app.get('/api/messages/random', wrap(async (req, res) => {
  const row = await db
    .prepare("SELECT id, content FROM messages WHERE status = 'approved' ORDER BY RANDOM() LIMIT 1")
    .get();
...
// Body: { content }
app.post('/api/messages', auth.requireAuth, wrap(async (req, res) => {
...
app.get('/api/favoris', auth.requireAuth, wrap(async (req, res) => {
  const rows = await db
    .prepare(
...
app.post('/api/favoris', auth.requireAuth, wrap(async (req, res) => {
  const { message_id } = req.body || {};
  const msg = await db.prepare("SELECT id FROM messages WHERE id = ? AND status = 'approved'").get(message_id);
  if (!msg) return res.status(404).json({ error: 'Message introuvable.' });
  // INSERT OR IGNORE : pas d'erreur si le favori existe déjà.
  await db.prepare('INSERT OR IGNORE INTO favoris (user_id, message_id) VALUES (?, ?)').run(
    req.user.id,
    message_id
  );
...
app.delete('/api/favoris/:id', auth.requireAuth, wrap(async (req, res) => {
  await db.prepare('DELETE FROM favoris WHERE user_id = ? AND message_id = ?').run(
    req.user.id,
    req.params.id
  );
  res.json({ ok: true });
...
app.get('/api/admin/pending', auth.requireAdmin, wrap(async (req, res) => {
  const rows = await db
    .prepare(
...
app.post('/api/admin/approve/:id', auth.requireAdmin, wrap(async (req, res) => {
  const info = await db
...
app.delete('/api/admin/reject/:id', auth.requireAdmin, wrap(async (req, res) => {
  await db
